...
with st.sidebar:  # Streamlit sidebar configuration
    st.title('Isosurface Plot')  # Add a title in the sidebar
...
fig = go.Figure(data=go.Isosurface(  # Create a 3D Isosurface plot
    ...
    surface_count=8,  # Set the number of isosurfaces to display
    colorscale='RdYlBu',  # Use the red-yellow-blue colormap for the isosurfaces
    caps=dict(x_show=False, y_show=False)  # Hide the caps at the boundaries of the x and y axes
    ))
...
st.plotly_chart(fig)  # Embed the 3D Isosurface plot in the Streamlit app
